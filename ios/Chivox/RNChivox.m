...
// 外部录音方法
RCT_EXPORT_METHOD(outerFeed:(nonnull NSDictionary *)options
                  resolve:(RCTPromiseResolveBlock)resolve
                  reject:(RCTPromiseRejectBlock)reject){
    if(nil == self.cloudengine){
      reject(@"0",@"chivox初始化失败", nil);
    ...
    //创建音频源
    ChivoxAIOuterFeed *outerFeed = [[ChivoxAIOuterFeed alloc] init];
    NSMutableString *tokenId = [[NSMutableString alloc] init];
    
    ChivoxAIRetValue * e = [self.cloudengine start:outerFeed tokenId:tokenId param:params listener:handler];
    if (0 != [e errId]){
      reject(@([e errId]).stringValue,@"chivoxOuterFeed操作失败", nil);
    } else {
      resolve(nil);
    }
}
...
// 外部录音 发送音频文件
RCT_EXPORT_METHOD(sendOuterFeedAudioFile:(nonnull NSString *)filePath
                  resolve:(RCTPromiseResolveBlock)resolve
                  reject:(RCTPromiseRejectBlock)reject) {
    ...
    char buf[2000];
    int length;
    while((length = (int)fread(buf, 1, 1024, file))){
        ChivoxAIRetValue *value = [self.cloudengine feed:buf length:length];
        if (0 != [value errId]){
